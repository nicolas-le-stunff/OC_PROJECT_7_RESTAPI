...
public class BidListControllerTest {
	...
	public void getBidListTest() throws Exception {
		...
		mockMvc.perform(get("/bidList/list")).andDo(print()).andExpect(status().isOk())
	...
	public void postNewBidTest() throws Exception {
		...
		mockMvc.perform(
				post("/bidList/validate").param("account", bidList.getAccount()).param("type", bidList.getType())
						.param("bidQuantity", String.valueOf(bidList.getBidQuantity())).with(csrf()))
				.andDo(print()).andExpect(view().name("redirect:/bidList/list")).andExpect(status().is3xxRedirection())
	...
	public void postNewBidWithErrorTest() throws Exception {
		mockMvc.perform(post("/bidList/validate").param("account", bidList.getAccount())
				// .param("type", bidList.getType())
				.param("bidQuantity", String.valueOf(bidList.getBidQuantity())).with(csrf())).andDo(print())
				.andExpect(view().name("bidList/add")).andExpect(status().is2xxSuccessful()).andReturn().getResponse()
				.containsHeader("accountTest");
	}
	...
	public void getBidListUpdateByExistingIdTest() throws Exception {
		...
		mockMvc.perform(get("/bidList/update/1")).andDo(print()).andExpect(status().isOk())
	...
	public void getBidListUpdateByUnknownIdTest() throws Exception {
		...
		mockMvc.perform(get("/bidList/update/5").param("account", "accountTest").param("type", "typeTest")
				.param("bidQuantity", "10").param("id", "5").with(csrf()))
	...
	public void postBidToUpdateTest() throws Exception {
		...
		mockMvc.perform(post("/bidList/update/1").param("account", bidList.getAccount())
				.param("id", String.valueOf(bidList.getId())).param("type", bidList.getType())
				.param("bidQuantity", String.valueOf(bidList.getBidQuantity())).with(csrf())).andDo(print())
	...
	public void postBidToUpdateWithErrorTest() throws Exception {
		...
		mockMvc.perform(post("/bidList/update/1").param("account", bidList.getAccount())
				// .param("type", bidList.getType())
				.param("bidQuantity", String.valueOf(bidList.getBidQuantity())).with(csrf())).andDo(print())
				.andExpect(view().name("bidList/update")).andExpect(status().isOk()).andExpect(model().hasErrors())
				.andExpect(model().attributeExists("bidList")).andReturn().getResponse()
	...
	public void getBidDeleteByIdTest() throws Exception {
		...
		mockMvc.perform(get("/bidList/delete/1")).andDo(print()).andExpect(status().is3xxRedirection())
